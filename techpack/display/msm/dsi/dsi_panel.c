...
/* Huaqin modify for DRDOOM-271 by gaozhengwei at 2020/10/30 start*/
bool esd_recovery_flag = false;
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	/* Huaqin modify for DRS-5026 by gaozhengwei at 2020/10/19 start*/
	if (esd_recovery_flag && r_config->esd_count) {

		DSI_INFO("panel IC is in sleep out mode, do esd recovery...\n");

		if (r_config->esd_count) {
			rc = gpio_direction_output(r_config->reset_gpio,
				r_config->esd_sequence[0].esd_level);
			if (rc) {
				DSI_ERR("unable to set esd recovery dir for rst gpio rc=%d\n", rc);
				goto exit;
			}
		}

		for (i = 0; i < r_config->esd_count; i++) {
			gpio_set_value(r_config->reset_gpio,
					r_config->esd_sequence[i].esd_level);


			if (r_config->esd_sequence[i].esd_sleep_ms)
				usleep_range(r_config->esd_sequence[i].esd_sleep_ms * 1000,
					(r_config->esd_sequence[i].esd_sleep_ms * 1000) + 100);
		}

		esd_recovery_flag = false;
		/* Huaqin modify for DRDOOM-271 by gaozhengwei at 2020/10/30 end*/
	} else {

		DSI_INFO("panel IC is in suspend, do resume...\n");
	...
	/* Huaqin modify for DRS-5026 by gaozhengwei at 2020/10/19 end*/
...
static int dsi_panel_power_on(struct dsi_panel *panel)
{
    ...
    #if defined(CONFIG_TOUCHSCREEN_ILI_7807G)
    if(is_ilitek_tp) {
        DSI_INFO("tp_7806G TP_RESUME\n");
		ilitek_resume_by_ddi();
	}
    #endif
    #if defined(CONFIG_TOUCHSCREEN_ILI_7806S)
    if(is_ilitek_tp_7806s) {
        DSI_INFO("tp_7806S TP_RESUME\n");
		ili_resume_by_ddi();
	}
    #endif
	/* Huaqin modify for DRSTRANGE-48 by gaozehngwei at 2020/06/03 end*/
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	/* Huaqin modify for DRSTRANGE-49 by yuanliding at 2020/05/27 start*/
	if (gpio_is_valid(panel->reset_config.reset_gpio) && !panel->reset_gpio_always_on)
		gpio_set_value(panel->reset_config.reset_gpio, 0);
	/* Huaqin modify for DRSTRANGE-49 by yuanliding at 2020/05/27 end*/
	...
	/* Huaqin modify for DRSTRANGE-49 by yuanliding at 2020/05/28 start*/
	if ((panel->reset_config.reset_delay > 0) && (panel->reset_config.reset_force_pull_low)){
		gpio_set_value(panel->reset_config.reset_gpio, 0);
		mdelay(panel->reset_config.reset_delay);
	}
	/* Huaqin modify for DRSTRANGE-49 by yuanliding at 2020/05/28 end*/
	...
	u32 bl_lvl)
{
	...
	/* Huaqin modify for SR-ZQL1838-01-138 by gaozhengwei at 2020/05/20 start*/
	struct dsi_backlight_config *bl = &panel->bl_config;
	/* Huaqin modify for SR-ZQL1838-01-138 by gaozhengwei at 2020/05/20 end*/
	...
	rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_CHANGE_PAGE0_BEFORE_W51);
	...
	/* Huaqin modify for SR-ZQL1838-01-138 by gaozhengwei at 2020/05/20 end*/
	/* Huaqin modify for DRSTRANGE-85 by yuanliding at 2020/05/25 start*/
	if (bl->bl_2bytes_0xff0f_enable)
		bl_lvl= ((bl_lvl &0x000f) << 8) |(bl_lvl >> 4);
	/* Huaqin modify for DRSTRANGE-85 by yuanliding at 2020/05/25 end*/
...
const char *cmd_set_prop_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-change-page0-before-W51-command",
};
...
const char *cmd_set_state_map[DSI_CMD_SET_MAX] = {
	...
	"qcom,mdss-dsi-change-page0-before-W51-command-state",
};
...
/* Huaqin modify for DRS-5026 by gaozhengwei at 2020/10/19 start*/
static int dsi_panel_parse_esd_recovery_reset_sequence(struct dsi_panel *panel)
{
	int rc = 0;
	int i;
	u32 length = 0;
	...
	arr = utils->get_property(utils->data,
			"qcom,mdss-dsi-esd-recovery-reset-sequence", &length);
	if (!arr) {
		DSI_ERR("[%s] dsi-esd-recovery-reset-sequence not found\n", panel->name);
		rc = -EINVAL;
		goto error;
	}
	if (length & 0x1) {
		DSI_ERR("[%s] syntax error for dsi-esd-recovery-reset-sequence\n",
		       panel->name);
		rc = -EINVAL;
		goto error;
	}
	...
	rc = utils->read_u32_array(utils->data, "qcom,mdss-dsi-esd-recovery-reset-sequence",
					arr_32, length);
	if (rc) {
		DSI_ERR("[%s] cannot read dsi-esd-recovery-reset-seqience\n", panel->name);
	...
	panel->reset_config.esd_sequence = seq;
	panel->reset_config.esd_count = count;

	for (i = 0; i < length; i += 2) {
		seq->esd_level = arr_32[i];
		seq->esd_sleep_ms = arr_32[i + 1];
		seq++;
	}
...
static int dsi_panel_parse_reset_sequence(struct dsi_panel *panel)
{
	...
	/* Huaqin modify for DRSTRANGE-49 by yuanliding at 2020/05/28 start*/
	rc = utils->read_u32(utils->data, "qcom,mdss-dsi-reset-delay", &val);
	if (rc) {
		panel->reset_config.reset_delay = 0;
		DSI_INFO("[%s] cannot read qcom,mdss-dsi-reset-delay\n", panel->name);
	} else {
		panel->reset_config.reset_delay = val;
		DSI_INFO("[%s] panel->reset_config.reset_delay is %d\n", panel->name,val);
	}
	/* Huaqin modify for DRSTRANGE-49 by yuanliding at 2020/05/28 end*/
...
static int dsi_panel_parse_gpios(struct dsi_panel *panel)
{
	...
	/* TODO:  release memory */
	/* Huaqin modify for DRS-5026 by gaozhengwei at 2020/10/19 start*/
	rc = dsi_panel_parse_esd_recovery_reset_sequence(panel);
	if (rc) {
		DSI_ERR("[%s] failed to parse esd recovery reset sequence, rc=%d\n",
			panel->name, rc);
	}
	/* Huaqin modify for DRS-5026 by gaozhengwei at 2020/10/19 end*/
