...
//#include <stdio.h>
#include <linux/fs.h>
char *s5k3l6_sensor_name;
/*hmd code add for camera node info by chengzhi at 2020.05.21 start*/
#undef CAM_MODULE_INFO_CONFIG
#define CAM_MODULE_INFO_CONFIG 1
#if CAM_MODULE_INFO_CONFIG
extern char *cameraModuleInfo[4];
#endif
/*hmd code add for camera node info by chengzhi at 2020.05.21 end*/
	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->pipeline_delay =
		probe_info->reserved;
	s_ctrl->sensordata->slave_info.sensor_name=probe_info->psensor_name;
...
int cam_sensor_match_id(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	const char *gc_sensor_name;
	char *module_sensor_name;
        struct cam_sensor_i2c_reg_setting *gc_write_setting;
         int gc_rc=0;
         int gc_i=0; 
	uint32_t gc5035_flag_and_module_id=0;
	static  struct cam_sensor_i2c_reg_array gc5035_reg[24] =
		{
			//otp init setting begin
			{0xfc,0x01,0,0},{0xf4,0x40,0,0},{0xf5,0xe9,0,0},{0xf6,0x14,0,0},
			{0xf8,0x49,0,0},{0xf9,0x82,0,0},{0xfa,0x10,0,0},{0xfc,0x81,0,0},
			{0xfe,0x00,0,0},{0x36,0x01,0,0},{0xd3,0x87,0,0},{0x36,0x00,0,0},
			{0xf7,0x01,0,0},{0xfc,0x8f,0,0},{0xfc,0x8f,0,0},{0xfc,0x8e,0,0},
			{0xfe,0x02,0,0},{0x55,0x80,0,0},{0x65,0x7e,0,0},{0x66,0x03,0,0},
			//init setting end
			{0x67,0xc0,0,0},//{0x68,0x11,0,0},
			{0xe0,0x1f,0,0},//0xe0 page address
			{0x67,0xf0,0,0},
			{0xf3,0x10,0,0},//  otp read pulse
			/*(if otp guide 0x1f98 0x1f is page 0x98 is
			address but need to change it .the ture address is (0xc0+(0x98/8))
			0x1f read data num */
			//{ 0x67,0xc0,0,0},//write end setting over read
			//{0xf3,0x00,0,0},//write end setting  over read
		};
	gc_write_setting=(struct cam_sensor_i2c_reg_setting *)vmalloc(sizeof(struct cam_sensor_i2c_reg_setting));
	slave_info = &(s_ctrl->sensordata->slave_info);
         gc_sensor_name=s_ctrl->sensordata->slave_info.sensor_name;
         CAM_ERR(CAM_SENSOR,"sensor_name =%s",s_ctrl->sensordata->slave_info.sensor_name);
	if (!slave_info) {
	...
	if((slave_info->sensor_id == 0x02e0)||(slave_info->sensor_id==0x5035)||(slave_info->sensor_id==0x8044)){
	...
	if((0 == strcmp(gc_sensor_name, "hmd1_gc8034_ly"))||(0 == strcmp(gc_sensor_name, "hmd2_gc8034_jk"))){//is gc8044
		s_ctrl->io_master_info.cci_client->sid=0xA0>>1;//0xA0 eeprom IIC address
		CAM_ERR(CAM_SENSOR,"hmd1_gc8034_ly cci_client->sid =%d",s_ctrl->io_master_info.cci_client->sid);
		camera_io_dev_read(&(s_ctrl->io_master_info),0x01,&gc5035_flag_and_module_id, CAMERA_SENSOR_I2C_TYPE_WORD,CAMERA_SENSOR_I2C_TYPE_BYTE);
		CAM_ERR(CAM_SENSOR,"gc8034_flag_and_module_id =%d",gc5035_flag_and_module_id);
		if((gc5035_flag_and_module_id==0x57)||(gc5035_flag_and_module_id==0x87))
			module_sensor_name="hmd1_gc8034_ly";
		else if(gc5035_flag_and_module_id==0x08)
			module_sensor_name="hmd2_gc8034_jk";
		s_ctrl->io_master_info.cci_client->sid=0x6E>>1;//0x6E sensor IIC address
		if(0!=strcmp(gc_sensor_name, module_sensor_name)){
		          vfree(gc_write_setting);
		          return -ENODEV;
		}
	}

	if((0 == strcmp(gc_sensor_name, "hmd2_gc5035_jk"))||(0 == strcmp(gc_sensor_name, "hmd1_gc5035_ly"))){//is gc5035
	for(gc_i=0;gc_i<=23;gc_i++){
		         gc_write_setting->addr_type=1;
			gc_write_setting->data_type=1;
			gc_write_setting->size=1;
			gc_write_setting->delay=5;
			gc_write_setting->reg_setting=&gc5035_reg[gc_i];
			gc_rc=camera_io_dev_write_continuous(&(s_ctrl->io_master_info),gc_write_setting,1);
		}
		camera_io_dev_read(&(s_ctrl->io_master_info),(0xc0+0x98/8),&gc5035_flag_and_module_id, CAMERA_SENSOR_I2C_TYPE_BYTE,CAMERA_SENSOR_I2C_TYPE_BYTE);
         	CAM_ERR(CAM_SENSOR," gc5035_flag=0x%x ",gc5035_flag_and_module_id); 
		if(gc5035_flag_and_module_id==0x4){
			camera_io_dev_read(&(s_ctrl->io_master_info),(0xc0+0xa0/8),&gc5035_flag_and_module_id, CAMERA_SENSOR_I2C_TYPE_BYTE,CAMERA_SENSOR_I2C_TYPE_WORD);
			CAM_ERR(CAM_SENSOR," gc5035_group1=0x%x ",gc5035_flag_and_module_id);
			if(gc5035_flag_and_module_id==0x5726)
				module_sensor_name="hmd1_gc5035_ly";
			else if(gc5035_flag_and_module_id==0x0826)
				module_sensor_name="hmd2_gc5035_jk";
		}
		else if(gc5035_flag_and_module_id==0xd){
			camera_io_dev_read(&(s_ctrl->io_master_info),(0xc0+0xd0/8),&gc5035_flag_and_module_id, CAMERA_SENSOR_I2C_TYPE_BYTE,CAMERA_SENSOR_I2C_TYPE_WORD);
			CAM_ERR(CAM_SENSOR," gc5035_group2=0x%x ",gc5035_flag_and_module_id); 
			if(gc5035_flag_and_module_id==0x5726)
				module_sensor_name="hmd2_gc5035_jk";
			else if(gc5035_flag_and_module_id==0x0826)
				module_sensor_name="hmd1_gc5035_ly";
	...
	/*hmd code add for camera node info by chengzhi at 2020.05.21 start*/
	#if CAM_MODULE_INFO_CONFIG
	cameraModuleInfo[s_ctrl->soc_info.index]=(char *)vmalloc(strlen(s_ctrl->sensordata->slave_info.sensor_name)+1);
	strlcpy(cameraModuleInfo[s_ctrl->soc_info.index],s_ctrl->sensordata->slave_info.sensor_name,strlen(s_ctrl->sensordata->slave_info.sensor_name)+1);
	CAM_ERR(CAM_SENSOR, "cameraModuleInfo[%d] :  sensor_name %s",
				s_ctrl->soc_info.index,s_ctrl->sensordata->slave_info.sensor_name);
	#endif
	/*hmd code add for camera node info by chengzhi at 2020.05.21 end*/
	return rc;
}
...
int cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	slave_info = &(s_ctrl->sensordata->slave_info);
	s5k3l6_sensor_name=slave_info->sensor_name;
