...
struct smb5 {
	...
	/*Huaqin added for DRSTRANGE-1054 Optimize AICL threshold by wangzikang at 2020/08/24 start */
...
static int __debug_mask = PR_MISC;	 
 #endif
	/*Huaqin added for DRSTRANGE-1054 Optimize AICL threshold by wangzikang at 2020/08/24 end */
...
static enum power_supply_property smb5_batt_props[] = {
	...
	/*Huaqin added for AR-ZQL1838-01-59  by wangzikang at 2020/05/19 start */
	 #if defined(HQ_FACTORY_BUILD) //factory version
	POWER_SUPPLY_PROP_CHARGING_ENABLED,
	#endif
	/*Huaqin added for AR-ZQL1838-01-59  by wangzikang at 2020/05/19 end */
	/*Huaqin added for DRSTRANGE-471 by wangzikang at 2020/07/22 start*/
	POWER_SUPPLY_PROP_RECHARGE_VBAT,
	/*Huaqin added for DRSTRANGE-471 by wangzikang at 2020/07/22 end*/
	/*Huaqin added for DRSTRANGE-656  by wangzikang at 2020/07/27 start */
	#if defined(HQ_FACTORY_BUILD) //factory version
	POWER_SUPPLY_PROP_BATT_CAPACITY_CONTROL,
	#endif
	/*Huaqin added for DRSTRANGE-656  by wangzikang at 2020/07/27 end */

};

#define DEBUG_ACCESSORY_TEMP_DECIDEGC	250
/*Huaqin added for DRSTRANGE-610  by wangzikang at 2020/07/06 start */
#if defined(HQ_FACTORY_BUILD) //factory version
#define FACTORY_STOP_CHARGING_CAPACITY	80
#endif
/*Huaqin added for DRSTRANGE-610  by wangzikang at 2020/07/06 end */
		...
		union power_supply_propval *val)
{
	...
	int rc = 0;
	/*Huaqin added for SR-ZQL1838-01-152  by wangzikang at 2020/05/21 start */
	#if defined(HQ_FACTORY_BUILD) //factory version
	union power_supply_propval pval = {0, };
	#endif
	/*Huaqin added for SR-ZQL1838-01-152  by wangzikang at 2020/05/21 end */
	switch (psp) {
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		/*Huaqin added for AR-ZQL1838-01-59  by wangzikang at 2020/05/19 start */
		#if defined(HQ_FACTORY_BUILD) //factory version
		if (1 == chg->batt_capacity_control_flag)
		{
			/*Huaqin added for DRSTRANGE-610  by wangzikang at 2020/07/06 start */
			if ( val->intval >= FACTORY_STOP_CHARGING_CAPACITY )
			/*Huaqin added for DRSTRANGE-610  by wangzikang at 2020/07/06 end */
			{
				/*Huaqin added for AR-ZQL1838-01-59 optimized  by wangzikang at 2020/06/15 start */
				if ( 0 != chg->charging_enable_flag)
				{
					pval.intval = 0;
					smblib_set_prop_battery_charging_enabled(chg, &pval);
					pr_info("The battery capacity is %d over FACTORY_STOP_CHARGING_CAPACITY! Charging disabled!\n",val->intval );
				}
			}
			else
			{
				if ( 1 != chg->charging_enable_flag)
				{
					pval.intval = 1;
					smblib_set_prop_battery_charging_enabled(chg, &pval);
					pr_info("The battery capacity is %d below FACTORY_STOP_CHARGING_CAPACITY, Charging enabled!\n", val->intval );
				}
				/*Huaqin added for AR-ZQL1838-01-59 optimized  by wangzikang at 2020/06/15 end */
			}
		}
		else
		{
			if (2 != chg->charging_enable_flag)
			{
				pval.intval = 1;
				smblib_set_prop_battery_charging_enabled(chg, &pval);
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (chip->dt.batt_profile_fv_uv < 0)
		smblib_get_charge_param(chg, &chg->param.fv,
				&chg->batt_profile_fv_uv);
	/*Huaqin added for DRSTRANGE-277 Modify AICL threshold by wangzikang at 2020/06/15 start */
	/*Huaqin added for DRSTRANGE-1054 Optimize AICL threshold by wangzikang at 2020/08/24 start */
	rc = smblib_set_charge_param(chg, &chg->param.aicl_5v_threshold, 4300);
	pr_info("AICL threshold is 4.3V.\n");
	/*Huaqin added for DRSTRANGE-1054 Optimize AICL threshold by wangzikang at 2020/08/24 end */
	/*Huaqin added for DRSTRANGE-277 Modify AICL threshold by wangzikang at 2020/06/15 end */
	...
	/*Huaqin added for DRSTRANGE-203 by wangzikang at 2020/06/10 start */
	 rc = smblib_write(chg, 0x154A, 0x17);
	 if (rc < 0) {
		dev_err(chg->dev, "Couldn't set 0x154A rc=%d\n", rc);
	}
	/*Huaqin added for DRSTRANGE-203 by wangzikang at 2020/06/10 end */
	return rc;
}
...
/* ZQL1838 added for DRSTRANGE-318 HARD JEITA threshold optimized by wangzikang at 2020/06/18 start */
struct smb_charger *smbchg_dev = NULL;
/* ZQL1838 added for DRSTRANGE-318 HARD JEITA threshold optimized by wangzikang at 2020/06/18 end */
